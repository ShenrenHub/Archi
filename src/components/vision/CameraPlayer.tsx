    ...
    <div className="relative overflow-hidden rounded-[24px] bg-slate-950">
      ...
      <div className="relative flex h-[300px] flex-col justify-between p-5 text-white">
        <div className="flex items-center justify-between">
          <span className="rounded-full bg-white/10 px-3 py-1 text-xs backdrop-blur">RTSP Stream Placeholder</span>
          <span className="rounded-full bg-emerald-500/20 px-3 py-1 text-xs text-emerald-200">1080P Online</span>
        </div>
        <div>
          <h4 className="text-2xl font-semibold">{title}</h4>
          <p className="mt-2 max-w-lg text-sm text-slate-300">
            预留实时视频流接入位，可替换为 WebRTC、HLS 或厂商 SDK 画面组件。
          </p>
        </div>
      </div>
    </div>
